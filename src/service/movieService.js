...
module.exports = class MovieService {
	...
	async getStats() {
		...
		Object.keys(yearsGroupedByProducer).forEach((producer) => {
			...
			yearsGroupedByProducer[producer] = (yearsGroupedByProducer[producer] || []).sort((a, b) => a - b);

			const producerIntervalMinMax = this.getProducerIntervalMinMax(producer, yearsGroupedByProducer[producer]);
			...
			producersByMin[producerIntervalMinMax.min.interval].push({
				producer,
				interval: producerIntervalMinMax.min.interval,
				previousWin: producerIntervalMinMax.min.previousWin,
				followingWin: producerIntervalMinMax.min.followingWin,
			});

			producersByMax[producerIntervalMinMax.max.interval].push({
				producer,
				interval: producerIntervalMinMax.max.interval,
				previousWin: producerIntervalMinMax.max.previousWin,
				followingWin: producerIntervalMinMax.max.followingWin,
			});
		});
	...
	getProducerIntervalMinMax(producer, years) {
		...
		const max = {
			producer,
			interval: Number.MIN_SAFE_INTEGER,
			previousWin: null,
			followingWin: null,
		};
		...
		while (l + 1 < years.length) {
			...
			if (delta < min.interval) {
				min.interval = delta;
				min.previousWin = years[l];
				min.followingWin = years[r];
			}

			if (delta > max.interval) {
				max.interval = delta;
				max.previousWin = years[l];
				max.followingWin = years[r];
			}
